...
escaladoresRouter.post('/escaladores', (request, response) => {
  ...
  const id =
    escaladores.Escaladores.length === 0
      ? 0
...
escaladoresRouter.get('/escaladores/:id', (request, response) => {
  ...
  const escalador = escaladores.Escaladores.find(
    (escaladorEnArray) => escaladorEnArray.id === parseInt(id, 10)
  );
...
escaladoresRouter.delete('/escaladores/:id', (request, response) => {
  ...
  let borrado = false;
  const escaladorIndex = escaladores.Escaladores.findIndex(
    (escaladorEnArray) => escaladorEnArray.id === parseInt(id, 10)
  );
